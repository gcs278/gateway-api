...
type SessionPersistence struct {
    ...
    // SessionPersistenceType defines the method for achieving session
    // persistence, such as through the use a header or cookie.
...
However, this API spec does allow specifying specific forms or types of session persistence through the
`SessionPersistenceType` field, including options like cookie-based or header-based session persistence. This API
addition accommodates implementations that offer multiple methods of session persistence, while also allowing users to
specify their preferred form of session persistence if needed.
